...
public class lifeCycleTestServlet extends HttpServlet {
     ...
     *     3. 서블릿도 클래스이므로 객체를 생성해 작동한다. 따라서 서버 구동 시 서블릿을 생성하면 톰캣 최초 수행 시에는 시간이 좀 걸리지만,
     *        클라이언트 요청에는 더 빠르게 응답할 수 있다.
     *
    ...
    // GET 요청은 주로 데이터를 요청하고 서버로부터 정보를 가져오는 데 사용됩니다. (예: http://localhost:8080/sendMessage?message=abc&code=20).
    // URL 에 파라미터를 포함할 수 있으며, 브라우저에서 직접 요청을 보낼 때 주로 사용된다.
    // HttpServlet 에서 상속받는다.
    @Override
    ...
    // POST 요청은 주로 서버에 데이터를 제출하거나 서버 상태를 변경하는 작업에 사용됩니다. 폼 데이터를 제출할 때 주로 사용됩니다.
